...
1. **Lock state (client-side, no new tables):** derive `locked` from the existing live-timeline current-block computation: current block is an activity (not screen/free) AND not ticked AND not passed/excused/outing.
