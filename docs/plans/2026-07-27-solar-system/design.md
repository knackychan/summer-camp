...
**Slices:** `30-solar-data.md` … `34-pixel-planet-textures.md` — 30: bilingual dataset (+ `type`/`desc`/`photo` fields per D8); 31: the living 3D scene with persistent time-warp band; 32: focus-zoom rig + holo wiki card; 33: quiz missions; 34: AI-assisted pixel-art planet textures (D9, any time after 31). Numbering continues the global sequence — admin-ops-redesign took 19–29, so the game platform's outstanding slice numbers (its own 19–22) are superseded in practice; this plan takes 30–34 and delivers the platform's "22 — real 3D game".
**Binding references:** `art-direction.md` (visual) and `tech-spec.md` (engineering) — deviation from either is a bug; changes require amending the document, never silent improvisation. `design-preview.html` illustrates but does not bind.
...
| D8 | **One living scene — Explore and Orbit are merged** (Papa, 2026-07-27, reviewing the 3D draft). Kids must be able to tap a planet at any time; mode-switching to interact makes no sense to them. So: (a) there is no Orbit mode — the time-warp band is persistent in the free scene; (b) tapping a planet **zooms the camera to it** (focus rig: target tracks the body, other bodies dim); (c) a **sci-fi "holo" info card** opens on the right with a **real photo**, a wiki-style bilingual description, a **rotating random fact** (🔀 reshuffles), and a stats grid; ✕ or double-tap zooms back out. Photos are **NASA public-domain images vendored into `assets/solar/`** and precached (offline-first holds; never hotlinked). 3D spheres stay flat-shaded — photography lives only inside the card. This supersedes D2's "three modes" (now two: free scene + quiz) and D3's "no assets" (card photos are in v1), and narrows art-direction §8's image/camera bans accordingly. Quiz mode suppresses focus/card (tap = answer). |
| D9 | **Pixel-art planet textures, AI-assisted, tool-gated** (Papa, 2026-07-27). The 3D planets get **pixel-art equirectangular albedo maps** ("Pixel Space" — 16-bit storybook look, art-direction §10) generated via an AI image pipeline. The pipeline is **tool-agnostic with a gate**: slice 34 opens by verifying what the configured Magnific MCP server can actually do (as of this design, no Magnific MCP is configured in this repo — only GitHub). Noted engineering constraint: Magnific is an *upscaler*, which fights crisp pixel art — if the gate shows generation isn't available, base maps come from Papa's chosen generator or hand-tooling, and Magnific's role shrinks to optionally enhancing the card's NASA photos. **The Sun stays untextured** (emissive + glow), the info card keeps real NASA photos, and every map is palette-locked to `solar-data.js` colours before vendoring. Slice 34 runs any time after slice 31; flat colour remains the automatic fallback if a map is missing. |
